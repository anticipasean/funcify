...
public class ErrableFunctionTypeTemplate<V, R> implements TypeGenerationTemplate<V, R> {

    private static final Logger logger = LoggerFactory.getLogger(ErrableFunctionTypeTemplate.class);

    @Override
    public List<String> getDestinationTypePackagePathSegments() {
        return Arrays.asList("funcify", "function", "errable");
    }
    ...
    public TypeGenerationSession<V, R> createTypesForSession(final TypeGenerationSession<V, R> session) {
        logger.debug("create_types_for_session: [ {} ]", SyncMap.empty().put("types", "ErrableFn[1..n]"));
        try {
            ...
            for (EnsembleKind inputEnsembleKind : ensembleKinds) {
                final int arity = inputEnsembleKind.getNumberOfValueParameters() - 1;
                ...
                final String className = "ErrableFn" + arity;
                final SyncMap<String, Object> params = SyncMap.of("package",
                                                                  ...
                                                                  "next_type_variable",
                                                                  CharacterOps.uppercaseLetterByIndexWithNumericExtension(
                                                                          inputEnsembleKind.getNumberOfValueParameters()),
                                                                  "ensemble_type_name",
                ...
                final StringTemplateSpec spec = DefaultStringTemplateSpec.builder()
                                                                         ...
                                                                         .typePackagePathSegments(
                                                                                 getDestinationTypePackagePathSegments())
                                                                         .templateFunctionName("errable_function_type")
            ...
            return session.withErrableFunctionTypeResults(results);
