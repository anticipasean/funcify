...
public class TraversableConjunctDesignTypeTemplate<V, R> implements TraitDesignGenerationTemplate<V, R> {

    private static final Logger logger = LoggerFactory.getLogger(TraversableConjunctDesignTypeTemplate.class);
    ...
    public List<String> getDestinationTypePackagePathSegments() {
        return Arrays.asList("funcify", "trait", "design", "traversable", "conjunct");
    }

    @Override
    public Path getStringTemplateGroupFilePath() {
        return Paths.get("antlr", "funcify", "ensemble", "trait", "design", "traversable", "traversable_conjunct_design_type.stg");
    }
    ...
    public TypeGenerationSession<V, R> createTypesForSession(final TypeGenerationSession<V, R> session) {
        logger.debug("create_types_for_session: [ {} ]",
                     SyncMap.empty().put("types", "TraversableConjunctEnsembleDesign[1..n]"));
        try {
            ...
            final SyncMap<EnsembleKind, WriteResult<R>> results = session.getConjunctTraversableDesignTypeResults();
            ...
            return session.withConjunctTraversableDesignTypeResults(results);
