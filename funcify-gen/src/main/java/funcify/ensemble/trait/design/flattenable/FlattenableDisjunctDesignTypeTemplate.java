...
public class FlattenableDisjunctDesignTypeTemplate<V, R> implements TraitDesignGenerationTemplate<V, R> {

    private static final Logger logger = LoggerFactory.getLogger(FlattenableDisjunctDesignTypeTemplate.class);
    ...
    public List<String> getDestinationTypePackagePathSegments() {
        return Arrays.asList("funcify", "trait", "design", "flattenable", "disjunct");
    }
    ...
    public TypeGenerationSession<V, R> createTypesForSession(final TypeGenerationSession<V, R> session) {
        logger.debug("create_types_for_session: [ {} ]", SyncMap.empty().put("types", "DisjunctFlattenableEnsembleDesign[1..n]"));
        try {
            ...
            final SyncMap<EnsembleKind, WriteResult<R>> results = session.getDisjunctFlattenableDesignTypeResults();
            ...
            return session.withDisjunctFlattenableDesignTypeResults(results);
